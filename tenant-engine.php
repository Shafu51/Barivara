...
if(!empty($_FILES['id_photo'])){
    ...
    if(isset($_FILES['pp']))
	{
$pp='tenant-photo/'.$_FILES['pp']['name'];

// echo $_FILES['image']['name'].'<br>';

if(!empty($_FILES['pp'])){
    $path2 = "tenant-photo/";
    $path2=$path2. basename($_FILES['pp']['name']);
        if(move_uploaded_file($_FILES['pp']['tmp_name'], $path2))
        {
	...
	global $tenant_id,$full_name,$email,$password,$phone_no,$address,$id_type,$id_photo,$pp,$errors,$db;
	...
	$password = md5($password); // Encrypt password
		$sql = "INSERT INTO tenant(tenant_id,full_name,email,password,phone_no,address,id_type,id_photo,pp) VALUES('$tenant_id','$full_name','$email','$password','$phone_no','$address','$id_type','$path','$path2')";
